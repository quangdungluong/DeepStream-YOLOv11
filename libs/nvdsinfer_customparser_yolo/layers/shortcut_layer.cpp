...
nvinfer1::ITensor *shortcutLayer(int layerIdx, std::string activation,
                                 std::string inputVol, std::string shortcutVol,
  ...
  if (inputVol != shortcutVol) {
    ...
    output = sliceLayer(layerIdx, name, shortcutInput, start, size, stride,
                        network, batchSize);
    assert(output != nullptr);
  } else
    output = shortcutInput;

  nvinfer1::IElementWiseLayer *shortcut = network->addElementWise(
      *input, *output, nvinfer1::ElementWiseOperation::kSUM);
