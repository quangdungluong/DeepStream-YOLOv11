...
nvinfer1::ITensor *poolingLayer(int layerIdx,
                                std::map<std::string, std::string> &block,
                                nvinfer1::ITensor *input,
                                nvinfer1::INetworkDefinition *network) {
  nvinfer1::ITensor *output;

  assert(block.at("type") == "max" || block.at("type") == "maxpool" ||
         block.at("type") == "avg" || block.at("type") == "avgpool");
  ...
  if (block.at("type") == "max" || block.at("type") == "maxpool") {
    ...
    nvinfer1::IPoolingLayer *maxpool = network->addPoolingNd(
        *input, nvinfer1::PoolingType::kMAX, nvinfer1::Dims{2, {size, size}});
