...
def set_custom_bbox(obj_meta):
    ...
    font_size = 18
    x_offset = int(
        min(STREAMMUX_WIDTH - 1, max(0, obj_meta.rect_params.left - (border_width / 2)))
    )
    y_offset = int(
        min(
            STREAMMUX_HEIGHT - 1, max(0, obj_meta.rect_params.top - (font_size * 2) + 1)
        )
    )
...
def videoconvert_src_pad_buffer_probe(pad, info, user_data):
    buf = info.get_buffer()
...
def main():
    ...
    converter_postosd = Gst.ElementFactory.make("nvvideoconvert", "convertor_postosd")
    if not converter_postosd:
        sys.stderr.write(" Unable to create converter_postosd \n")
    ...
    sys.stdout.write(
        "PERF_MEASUREMENT_INTERVAL_SEC: %d\n" % PERF_MEASUREMENT_INTERVAL_SEC
    )
    ...
    pipeline.add(osd)
    pipeline.add(converter_postosd)
    pipeline.add(caps)
    ...
    pgie.link(converter)
    converter.link(osd)
    osd.link(converter_postosd)
    converter_postosd.link(caps)
    caps.link(encoder)
    encoder.link(rtppay)
    rtppay.link(sink)
    ...
    videoconvert_src_pad = converter.get_static_pad("src")
    if not videoconvert_src_pad:
        sys.stderr.write("ERROR: Failed to get tracker src pad\n")
        sys.exit(1)
    else:
        videoconvert_src_pad.add_probe(
            Gst.PadProbeType.BUFFER, videoconvert_src_pad_buffer_probe, 0
        )
...
def parse_args():
    global SOURCE, CONFIG_INFER, CODEC, BITRATE, ENC_TYPE, STREAMMUX_BATCH_SIZE, STREAMMUX_WIDTH, STREAMMUX_HEIGHT, GPU_ID, PERF_MEASUREMENT_INTERVAL_SEC
    ...
    parser.add_argument(
        "--bitrate", default=2000000, help="Set the encoding bitrate ", type=int
    )
    parser.add_argument(
        "-e",
        "--enc_type",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Streammux batch-size (default: 1)",
    )
    parser.add_argument(
        "--streammux-width",
        type=int,
    ...
    parser.add_argument(
        "-g", "--gpu-id", type=int, default=0, help="GPU id (default: 0)"
    )
    parser.add_argument(
        "-f",
        "--fps-interval",
        type=int,
        default=5,
        help="FPS measurement interval (default: 5)",
    )
